...
//This class shamelessly copied and modified for this mod from Halation
public class SpideyTargetAction implements RenderSubscriber, PostUpdateSubscriber {
    private AbstractRelic r;

    public SpideyTargetAction(AbstractRelic r)
    {
        this.r = r;
        this.amount = amount;
    ...
    private void updateTargetMode()
    {
        ...
        if (InputHelper.justClickedLeft) {
        	...
        	if (this.hoveredCreature != null && this.hoveredCreature != AbstractDungeon.player) {
        		if (PizzaDeliveryRelic.ID.equals(r.relicId)) {
        			PizzaDeliveryRelic pd = (PizzaDeliveryRelic)r;
        			pd.deliverPizza(hoveredCreature);
        			com.megacrit.cardcrawl.core.GameCursor.hidden = false;
        			close();
        		}
        	}
    ...
    public void renderTargetingUi(SpriteBatch sb)
    {
        ...
        this.controlPoint = new Vector2(r.currentX - (x - r.currentX) / 4.0F, r.currentY + (y - r.currentY - 40.0F * Settings.scale) / 2.0F);
        if (this.hoveredCreature == null) {
        ...
        drawCurvedLine(sb, new Vector2(r.currentX, r.currentY), new Vector2(x, y), this.controlPoint);
