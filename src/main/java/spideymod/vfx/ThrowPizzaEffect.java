...
public class ThrowPizzaEffect extends AbstractGameEffect{
	  ...
	  public static final Texture IMG = TextureLoader.getTexture(SpideyMod.makeEventPath("Pizza.png"));
	
	public ThrowPizzaEffect(float srcX, float srcY, float destX, float destY) {
	    if (img == null) {
	        img = ImageMaster.vfxAtlas.findRegion("combat/pizza");
	      }
	      this.sX = srcX;
	      this.sY = srcY;
	      this.cX = this.sX;
	      ...
	      this.color = new Color(1.0F, 0.8F, 0.0F, 0.5F);
	      if (this.sY > this.dY) {
	  ...
	  public void render(SpriteBatch sb) {
	    ...
	    for (int i = 5; i < this.previousPos.size(); i++) {
	      sb.draw(ImageMaster.POWER_UP_2, 
	        ...
	        ((Vector2)this.previousPos.get(i)).x - img.packedWidth / 2, 
	        ((Vector2)this.previousPos.get(i)).y - img.packedHeight / 2, img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth, img.packedHeight, this.scale / (40.0F / i), this.scale / (40.0F / i), this.rotation);
	    }
	    sb.setColor(this.color);
	    sb.draw(img, this.cX - img.packedWidth / 2, this.cY - img.packedHeight / 2 + this.yOffset, img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth, img.packedHeight, this.scale, this.scale, this.rotation);
